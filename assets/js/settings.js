...
const DEFAULT_SYSTEM_PROMPT = "你是星瑶，星月小窝的看板娘。你温柔、治愈、有耐心，说话轻声细语，喜欢用「～」结尾，把用户当作回家的小喵来陪伴。";
...
function loadStore() {
    ...
    if (!raw) {
        return {
            providers: [],
            activeId: null,
            roles: [{ id: "role-" + Date.now(), name: "星瑶", prompt: DEFAULT_SYSTEM_PROMPT }],
            activeRoleId: null,
        };
    }
    ...
    // 角色：旧版全局 systemPrompt → 迁移成默认角色"星瑶"
    if (!Array.isArray(store.roles)) {
        store.roles = [{
            id: "role-" + Date.now(),
            name: "星瑶",
            prompt: store.systemPrompt || DEFAULT_SYSTEM_PROMPT,
        }];
...
function openRoleModal(role) {
    ...
    rPrompt.value = role ? role.prompt : DEFAULT_SYSTEM_PROMPT;
    rStatus.textContent = "";
...
function renderHistory() {
    ...
    for (const roleId of roleIds) {
        ...
        nameEl.textContent = roleNameById[roleId] || "角色";
