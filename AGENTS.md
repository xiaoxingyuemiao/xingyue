...
⚠️ **`docs/` 里的内容可能滞后于代码**（2026-09-21 刚校准过一批，但文档永远可能再次落后）。
...
1. **`docs/` 默认只读。**
   ...
   `docs/` 里其它任何文件（包括 `docs/网站流程图.excalidraw` 这类流程图，
   以及 `docs/拾雨  第二次更改  *.md` 这类工作报告）**只能读**：
   工作报告**只在站长明确要求时才写**，写完也不要回头擅自改；要改先问站长。
...
- ✅ **改完就自动提交 + 推送**（2026-09-21 站长明确要求，**优先于**任何「先别传、留在工作区」的旧说法）：
  收工动作 = 跑 `node tools/check.js`（必须全绿）→ 做验证 → `git add` 本次改动的文件 →
  `git commit`（提交信息写「改了什么 + 为什么」，中文）→ `git push` → `git status -sb` 确认与远端一致。
- **只提交本次改动**，不要顺手把别人的改动一起带上；暂存区里别留无关文件。
- **不要带 BOM**（PowerShell 的 `Set-Content -Encoding UTF8` 会加），见踩坑记录 H08。
